...
int main (void)
{
    ...
    char сору [81];
    char choice;
    //указывает на функцию, которая использует значение типа char * в качестве аргумента и не имеет возвращаемого значения 
    void (*pfun) (char *);
    ...
    while (gets (line) != NULL && line [0] != '\0')
    {
        while ((choice = showmenu ()) != 'n')
        {
            ...
            strcpy (сору, line);                                                //копия для функции show ()
            show (pfun, сору);                                                  //используется избранная функция
        }
        puts ("Введите строку (пустая строка - выход из программы): ");
    }
    puts ("Bceгo доброго!");
